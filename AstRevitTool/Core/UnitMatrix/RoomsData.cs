        ...
        public DataTable GenRoomsDataTable(Level level)
        {
            ...
            foreach (Room room in m_rooms)
            {
                ...
                if ((null == level) || (m_activeDocument.GetElement(room.LevelId) != null && room.LevelId == level.Id))
                {
                    ...
                    Export.SvgExport.roomJson rjson;
                    Util.DetermineAdjacentElementLengthsAndWallAreas(room, out rjson);
                    if (rjson != null)
                    {
                        dataRow[m_parameters.Count + 1] = Math.Round(rjson.ajustedRoomArea);
                    }
                    else
                    {
                        dataRow[m_parameters.Count+1] = "<null>";
                    }
                    // add this row
