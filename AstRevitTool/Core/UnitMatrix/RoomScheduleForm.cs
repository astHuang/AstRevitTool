...
namespace AstRevitTool.Core.UnitMatrix
{
    ...
    public partial class RoomScheduleForm : System.Windows.Forms.Form
    {
        ...
        private void UpdateFormDisplay(bool bUpdateAllRooms)
        {
            ...
            checkBoxColumn.HeaderText = "Export";
            ...
            revitRoomDataGridView.Columns.Insert(0, checkBoxColumn);
        ...
        private void UpdateRoomMapSheetInfo()
        {
            ...
            if (Application.EventReactor.DocMappedSheetInfo(hashCode, ref xlsAndTable))
            {
        ...
        private void tablesComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int hashCode = m_document.GetHashCode();
            if (Application.EventReactor.DocMonitored(hashCode))
            {
                // update spread sheet to which document is being mapped.
                Application.EventReactor.UpdateSheeInfo(hashCode, new SheetInfo(m_dataBaseName, m_roomTableName));
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string folder = "C:\\Users\\huang\\AppData\\Roaming\\Autodesk\\Revit\\Addins\\2020\\AstRevitTool\\BOMA";
            string appfolder = folder + "\\dist\\app";
            string datafolder = appfolder + "\\data";
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                StringBuilder sb = new StringBuilder();

                for (int i = 0; i < revitRoomDataGridView.Columns.Count; i++)
                {
                    sb.Append('"');
                    sb.Append(revitRoomDataGridView.Columns[i].HeaderText);
                    sb.Append('"');
                    if (i < revitRoomDataGridView.Columns.Count - 1)
                    {
                        sb.Append(" ");
                    }
                }
                sb.AppendLine();
                foreach (DataGridViewRow row in revitRoomDataGridView.Rows)
                {
                    if (row.Cells["Export"].Value != null && (bool)row.Cells["Export"].Value)
                    {
                ...
                string txtname = Path.GetFileNameWithoutExtension(saveFileDialog.FileName).Replace(" ", "") + ".txt";
                string title = this.m_document.Title;
        ...
        {
            foreach (DataGridViewRow row in revitRoomDataGridView.SelectedRows)
            {
                row.Cells["Export"].Value = true; // Or false to uncheck the checkbox
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewRow row in revitRoomDataGridView.Rows)
            {
                row.Cells["Export"].Value = false; // Or false to uncheck the checkbox
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                string txtname = Path.GetFileNameWithoutExtension(saveFileDialog.FileName).Replace(" ", "") + ".txt";
                string title = this.m_document.ProjectInformation.BuildingName;
                string txtpath = datafolder + "\\" + txtname;
                string mode = "UNITMATRIX";
                File.WriteAllText(txtpath, sb.ToString());
                string output = "Unit data exported! Now running script to generate Excel table...";
